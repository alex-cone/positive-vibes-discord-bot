    ...
    execute(message, args) {
        if (message.member.roles.has(message.guild.roles.find(role => role.name === "Veterans").id) || message.member.roles.has(message.guild.roles.find(role => role.name === "Officer").id)) {
            if (today.getDay() === 5 || today.getDay() === 6 ) {
                ...
                message.channel.send("Attendance tracking for today has been cancelled. Please remember if you want to cancel for another day, you must use this command again on that day.")
            } else {
