    ...
    wcl.getRankingsCharacter(
      ...
      function(err, data) {
        ...
        if ((!data || data.length === 0) && !params.partition) {
          console.log(
            "Could not find any parses for the latest patch. Trying an earlier one..."
          );
          params.partition = 1;
          getAParse(args, params, callback);
        } else {
          console.log("Found parse data...");
          const percentile = data.find(
            enc => enc.encounterName === args.bossName
          ).percentile;
          ...
          if (params.partition) {
            callback(
              "You are at the " +
                                percentile +
                                "th percentile of players of your spec for this fight on your best attempt." +
                                " Your rank for this fight is " +
                                rank +
                                ". Note that this is a parse from an earlier patch"
            );
          } else {
            callback(
              "You are at the " +
        ...
        if ((!data || data.length === 0) && !params.partition) {
          ...
          data.forEach(enc => {
            if (!encounters.get(enc.encounterName)) {
              encounters.set(enc.encounterName, {
                percentile: enc.percentile,
                rank: enc.rank,
                difficulty: enc.difficulty,
                reportID: enc.reportID
              });
            }
          });
          ...
          encounters.forEach(function(enc, key) {
            ...
            result =
                            result + "Percentile: " + enc.percentile + "\n";
            result = result + "Rank: " + enc.rank + "\n\n";
          });
          if (params.partition) {
            callback(
              "No current patch data found... Using older patch data for damage parse: \n\n" +
                                result
            );
          } else {
...
function getGuildAttendance() {
  wcl.getReportsGuild("Chicken Dinner", "Kiljaeden", "us", {}, function(
    err,
    data
  ) {
    if (err) {
